...
void main(int argc, char **argv){
  ...
  //sdkStartTimer(&timer);
  double timer, timer1, stimer1, stimer, stop, stop1;
  timer = MPI_Wtime(); 
  //loading the image
  ...
  //read in from file
  FILE *imf = fopen("news1080.data", "rb");
  fread(hData, sizeof(float), width*height, imf);	

  unsigned char *hOutputData = (unsigned char *)malloc(size);
  //double timer1, stimer1, stop, stop1;
...
MPI_Barrier(MPI_COMM_WORLD);
	//sdkStopTimer(&timer1);
        stop1 = MPI_Wtime() - timer1;

if(process_id == 0){

 
  //write image to file
  FILE *zz = fopen("out_mpi.data", "wb");
  fwrite(hData, sizeof(float), width*height, zz);
  fclose(zz);

 // cudaMemcpy(hOutputData, dOut, size, cudaMemcpyDeviceToHost);
  //char* imagePath = "/home/vmuser/Downloads/HPC\ PROJECT/";
  //strcpy(outputFilename, imagePath);
  //strcpy(outputFilename + strlen(imagePath) - 4, "_out_test_MPI.pgm");
  //sdkSavePGM(outputFilename, hOutputData, width, height);
  //sdkStopTimer(&timer);
  stop = MPI_Wtime() - timer;
    printf("Processing time: %f (ms)\n", stop1);
    printf("%.2f Mpixels/sec\n",
           ((width *height) / (stop1*1000 / 1000.0f)) / 1e6);
